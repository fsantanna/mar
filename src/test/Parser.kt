...
class Parser {
    ...
    fun aa_02_var_err () {
        G.tks = (" { ").lexer()
        parser_lexer()
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 2) : expected expression : have \"{\"")
    }
    @Test
    fun aa_03_var_err () {
        G.tks = ("  ").lexer()
        parser_lexer()
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 3) : expected expression : have end of file")
    }
    @Test
    fun aa_04_evt () {
        G.tks = (" evt ").lexer()
    ...
    fun bb_02_expr_parens_err() {
        val l = lexer(" ( a  ")
        val parser = Parser(l)
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 7) : expected \")\" : have end of file")
    }
    @Test
    ...
    fun dd_04_call_err() {
        val l = lexer("f (999 ")
        val parser = Parser(l)
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 8) : expected \",\" : have end of file")
    }
    @Test
    fun dd_05_call_err() {
        val l = lexer(" f ({ ")
        val parser = Parser(l)
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 5) : expected expression : have \"{\"")
    }
    ...
    fun ee_03_tuple_err() {
        val l = lexer("[{")
        val parser = Parser(l)
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 2) : expected expression : have \"{\"")
    }
    @Test
    fun ee_04_tuple() {
        val l = lexer("[1.5F,] ")
    ...
    fun dict3_err() {
        val l = lexer("@[({")
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 4) : expected expression : have \"{\"")
    }
    @Test
    fun dict4_err() {
        val l = lexer("@[(")
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 4) : expected expression : have end of file")
    }
    @Test
    fun dict5_err() {
        val l = lexer("@[(1,{")
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 6) : expected expression : have \"{\"")
    }
    @Test
    fun dict6() {
        val l = lexer("@[(1.5F,1),] ")
    ...
    fun dict7_err() {
        val l = lexer("@[(1,1]")
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 7) : expected \")\" : have \"]\"")
    }
    ...
    fun index2_err() {
        val l = lexer("x[10")
        val parser = Parser(l)
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 5) : expected \"]\" : have end of file")
    }
    @Test
    fun index4_err() {
        val l = lexer("x . a")
        ...
        //assert(trap { parser_exprSufs() } == "anon : (lin 1, col 5) : expected \"pub\" : have \"a\"")
    }
    @Test
    fun index5_err() {
        val l = lexer("x . .")
        val parser = Parser(l)
        //assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 5) : expected identifier : have \".\"")
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 5) : invalid field : unexpected \".\"")
    }
    @Test
    fun index6_err() {
        val l = lexer("x . 2")
        val parser = Parser(l)
        assert(trap { parser_expr_4_suf() } == "anon : (lin 1, col 5) : expected identifier : have \"2\"")
    }
    ...
    fun exprs_seq2() {
        ...
        val es = parser_exprs()
        assert(es.to_str() == "f();\n1;\nh();\ni();\n") { es.to_str() }
        //assert(trap { parser_exprs() } == "anon : (lin 2, col 3) : expression error : innocuous expression")
    }
    @Test
    fun exprs_seq2a() {
        val l = lexer("; f () \n ;;;do;;; (1) ; h()\ni() ;\n;")
        ...
        val es = parser_exprs()
        assert(es.to_str() == "f();\n1;\nh();\ni();\n") { es.to_str() }
        //assert(ceu.trap { parser_exprs() } == "anon : (lin 2, col 3) : call error : \"(\" in the next line")
    }
    @Test
    fun exprs_seq3() {
        val l = lexer("var v2\n[tp,v1,v2]")
    ...
    fun expr_dcl_err() {
        val l = lexer("var [10]")
        val parser = Parser(l)
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 5) : expected identifier : have \"[\"")
    }
    @Test
    fun expr_dcl3() {
        val l = lexer("var x = 1")
    ...
    fun expr_err1() {  // set number?
        ...
        assert(trap { parser_expr() } == "anon : (lin 1, col 1) : set error : expected assignable destination")
    }
    @Test
    ...
    fun set_evt_err() {
        ...
        val parser = Parser(l)
        assert(parser_expr() is Expr.Set)
        //assert(trap { parser_expr() } == "anon : (lin 1, col 1) : set error : expected assignable destination")
    }
    @Test
    fun set_err_err() {
    ...
    fun expr_if2_err() {
        ...
        //val e = parser_expr_prim()
        //assert(e is Expr.If)
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 14) : expected \"else\" : have end of file")
    }
    @Test
    fun if3() {
        ...
        //assert(trap { parser_expr_prim() } == "anon : (lin 2, col 16) : access error : variable \"f\" is not declared")
    }
    ...
    fun expr_do1_err() {
        ...
        //assert(e.tostr() == "do {\n\n}") { e.tostr() }
        assert(trap { parser_expr_prim() } == "anon : (lin 1, col 4) : expected expression : have \"}\"")
    }
    @Test
    fun expr_do2() {
    ...
    fun expr_func1_err() {
        ...
        //assert(trap { parser_expr_prim() } == "anon : (lin 1, col 10) : expected expression : have \"}\"")
    }
    @Test
    fun expr_func2() {
    ...
    fun pp_07_func_args_err() {
        ...
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 8) : expected identifier : have \"1\"")
    }
    @Test
    fun pp_12_func_nested() {
        ...
        assert(trap { parser_expr() } == "anon : (lin 2, col 19) : expected \"(\" : have \":nested\"")
    }
    @Test
    fun pp_13_minus() {
        val l = lexer("""
    ...
    fun pq_01_rec() {
        ...
        """)
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 2, col 32) : expected expression : have \"}\"")
    }
    @Test
    fun pq_02_rec() {
        val l = lexer("""
    ...
    fun native2_err() {
        ...
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 3, col 1) : native error : expected \"``\"")
    }
    @Test
    fun native3_err() {
        val l = lexer(
        ...
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 4, col 9) : native error : expected \"``\"")
    }
    @Test
    fun native4_err() {
        val l = lexer(
        ...
        val parser = Parser(l)
        assert(trap { parser_expr() } == "anon : (lin 1, col 8) : tag error : expected identifier")
    }
    @Test
    fun native5() {
        val l = lexer(
    ...
    fun bin1_err() {
        val l = lexer("(10+)")
        val parser = Parser(l)
        assert(trap { parser_expr_1_bin() } == "anon : (lin 1, col 5) : expected expression : have \")\"")
    }
    @Test
    fun bin2() {
        val l = lexer("10+1")
    ...
    fun tplate02_err() {
        ...
        val parser = Parser(l)
        assert(trap { parser_exprs() } == "anon : (lin 2, col 18) : expected tag : have \"X\"")
    }
    @Test
    fun tplate03_err() {
        val l = lexer(
        ...
        val parser = Parser(l)
        assert(trap { parser_exprs() } == "anon : (lin 2, col 21) : expected \"=\" : have \"[\"")
    }
    @Test
    fun tplate04_err() {
        val l = lexer(
        ...
        val parser = Parser(l)
        assert(trap { parser_exprs() } == "anon : (lin 2, col 23) : expected \"[\" : have \"nil\"")
    }
    @Test
    fun tplate05_err() {
        val l = lexer(
        ...
        val parser = Parser(l)
        assert(trap { parser_exprs() } == "anon : (lin 2, col 24) : expected identifier : have \"1\"")
    }
    @Test
    fun tplate06() {
        val l = lexer(
    ...
    fun tt_01_export_err() {
        ...
        val parser = Parser(l)
        //assert(trap { parser_expr() } == "anon : (lin 1, col 7) : expected \"[\" : have \"{\"")
        assert(trap { parser_expr() } == "anon : (lin 1, col 8) : expected expression : have \"}\"")
    }
    @Test
    fun BUG_tt_02_export_err() {
        val l = lexer("group ;;;[:x];;; {}")
        val parser = Parser(l)
        //assert(trap { parser_expr() } == "anon : (lin 1, col 9) : expected identifier : have \":x\"")
        assert(trap { parser_expr() } == "anon : (lin 1, col 19) : expected expression : have \"}\"")
    }
    @Test
    fun tt_03_export() {
    ...
    fun yy_01_innoc() {
        ...
        val e = parser_exprs()
        assert(e.to_str() == "do {\n1;\n};\nnil;\n") { e.to_str() }
        //assert(trap { parser_exprs() } == "anon : (lin 2, col 13) : expression error : innocuous expression")
    }
    @Test
    fun yy_02_innoc() {
        val l = lexer("""
    ...
    fun yy_03_innoc() {
        ...
        val parser = Parser(l)
        val e = parser_exprs()
        //assert(trap { parser_exprs() } == "anon : (lin 3, col 17) : expression error : innocuous expression")
    }
     */
}
