...
fun Type.Proto.Coro.x_coro_exec (tpl: Tpl_Map?, pre: Boolean): Pair<String,String> {
    val tps = (this.inps.to_void() + listOf(this.res,this.yld,this.out)).map { it.coder(tpl,pre) }.joinToString("__").clean()
...
fun Type.Exec.x_exec_coro (tpl: Tpl_Map?, pre: Boolean): Pair<String,String> {
    val tps = (this.inps.to_void() + listOf(this.res,this.yld,this.out)).map { it.coder(tpl,pre) }.joinToString("__").clean()
...
fun Var_Type.coder (tpl: Tpl_Map?, pre: Boolean): String {
    val (id,tp) = this
    return tp.coder(tpl,pre) + " " + id.str
}
fun Type.coder (tpl: Tpl_Map?, pre: Boolean): String {
    return when (this) {
        //is Type.Err,
        is Type.Any -> TODO()
        ...
        is Type.Unit       -> "_VOID_"
        is Type.Pointer    -> this.ptr.coder(tpl,pre) + (this.ptr !is Type.Proto).cond { "*" }
        is Type.Tuple      -> "Tuple__${this.ts.map { (id,tp) -> tp.coder(tpl,pre)+id.cond {"_"+it.str} }.joinToString("__")}".clean()
        is Type.Union      -> "Union__${this.ts.map { (id,tp) -> tp.coder(tpl,pre)+id.cond {"_"+it.str} }.joinToString("__")}".clean()
        is Type.Vector     -> "Vector__${this.max.cond2({it.tk.str},{"0"})}_${this.tp.coder(tpl,pre)}".clean()
        is Type.Proto.Func -> "Func__${this.inps.to_void().map { it.coder(tpl,pre) }.joinToString("__")}__${this.out.coder(tpl,pre)}".clean()
        is Type.Proto.Coro -> this.x_coro_exec(tpl,pre).first
        is Type.Exec       -> this.x_exec_coro(tpl,pre).first
    }
}
...
fun coder_types (pre: Boolean): String {
    fun ft (me: Type): List<String> {
        ...
        val tpls: List<Tpl_Map?> = if (s==null || s.tpls.isEmpty()) listOf(null) else {
            G.tpls[s]?.map { s.tpls.map{(id,_)->id.str}.zip(it).toMap() } ?: emptyList()
        }
            ...
            when (me) {
                ...
                is Type.Vector -> {
                    ...
                    listOf("""
                        typedef struct $x {
                            int max, cur;
                            ${me.tp.coder(tpl,pre)} buf[${me.max.cond2({it.tk.str},{""})}];
                        } $x;
                    """)
                }
                is Type.Union -> {
        ...
        return when (me) {
            is Expr.Bin -> {
                if (me.tk.str == "++") {
                    ...
                    val x = tp.coder(TODO(),pre)
                    if (G.types.contains(x)) {
                        return emptyList()
                    } else {
                        G.types.add(x)
                    }
                    val y = tp.tp.coder(TODO(),pre)
                    return listOf("""
                        typedef struct $x {
                            int max, cur;
                            $y buf[${tp.max!!.tk.str}];
        ...
        return when (me) {
                ...
                (me.subs == null) -> {
                    fun f(tp: Type, s: List<String>): List<String> {
                        ...
                        val tpls: List<Tpl_Map?> = if (me.tpls.isEmpty()) listOf(null) else {
                            G.tpls[me]?.map { me.tpls.map{(id,_)->id.str}.zip(it).toMap() } ?: emptyList()
                        }
                        return tpls.map { tpl ->
                            val x1 = "typedef ${tp.coder(tpl,pre)} $ss;\n"
                    ...
                    fun g (sup: Pair<String,String>?, s: Stmt.Data, I: Int): String {
                        ...
                        val flds = sup.cond { it.second } + tup.ts.mapIndexed { i, id_tp ->
                            ...
                            """
                            union {
                                ${tp.coder(TODO(),pre)} _${I+i};
                                ${id.cond { "${tp.coder(TODO(),pre)} ${it.str};" }}
                            };
                            """
                        }.joinToString("")
                        val subs = s.subs!!.map {
            ...
            is Stmt.Proto.Coro -> {
                    fun mem (): String {
                        ...
                        return blks.map { it.to_dcls().map { (_,id,tp) -> Pair(id,tp!!).coder(TODO(),pre) + ";\n" } }.flatten().joinToString("")
                    }
                    val (co,exe) = me.tp_.x_coro_exec(TODO(),pre)
                    listOf("""
                    typedef struct $exe {
                        int pc;
                        $co co;
    ...
    return when (this) {
            ...
            } + """
            {
                ...
                ${when {
                    ...
                    (this is Stmt.Proto.Coro) -> {
                        val (xuni,_) = this.tp_.x_out_uni(TODO(),pre)
            ...
            { // BLOCK | ${this.dump()}
                ...
                do {
                    ${this.to_dcls().map { (_,id,tp) ->
                        when (tp) {
                            is Type.Proto.Func -> "auto " + tp.out.coder(TODO(),pre) + " " + id.str + " (" + tp.inps.map { it.coder(TODO(),pre) }.joinToString(",") + ");\n"
                            is Type.Proto.Coro -> "auto ${tp.x_sig(TODO(), pre, id.str)};\n"
                            else -> ""
                        }
                    }.joinToString("")}
                    $body
        ...
        is Stmt.Dcl -> {
            val dcl = if (this.up_first { it is Stmt.Proto } is Stmt.Proto.Coro) "" else {
                this.xtp!!.coder(TODO(),pre) + " " + this.id.str + ";"
            }
        ...
        is Stmt.SetE    -> {
            ...
            when {
                ...
                (tdst is Type.Vector) -> """
                    {
                        ...
                        *mar_$n = CAST(${tdst.coder(TODO(),pre)}, $src);
                ...
                tdst.is_same_of(tsrc) -> "$dst = $src;"
                    ...
                    "$dst = CAST(${tdst.coder(TODO(),pre)}, $src);"
                }
            }
        }
        ...
        is Stmt.Catch -> {
            val uni  = this.type()?.coder(TODO(),pre)
            ...
            { // CATCH | ${this.dump()}
                ...
                if (MAR_EXCEPTION.tag == __MAR_EXCEPTION_NONE__) {
                    ...
                    ${(this.xup is Stmt.SetS && this.tp!=null).cond {
                        val set = this.xup as Stmt.SetS
                        """
                        ${set.dst.coder(pre)} = ($uni) { .tag=${xuni}_ERR_TAG, .Err=CAST(${this.tp!!.coder(TODO(),pre)}, MAR_EXCEPTION) };
                        """
                     }}
        ...
        is Stmt.Create -> {
            val xtp = (this.xup as Stmt.SetS).dst.typex().coder(TODO(),pre)
            (this.xup is Stmt.SetS).cond {
                val set = this.xup as Stmt.SetS
                """
                ${set.dst.coder(pre)} =
        ...
        is Stmt.Start -> {
            ...
            val (xuni,_) = tp.x_inp_uni(TODO(),pre)
            (this.xup is Stmt.SetS).cond {
                val set = this.xup as Stmt.SetS
                """
                ${set.dst.coder(pre)} =
        ...
        is Stmt.Yield -> {
            ...
            val (xuni,_) = tp.x_out_uni(TODO(),pre)
            """
                mar_exe->pc = ${this.n};
...
fun Expr.coder (pre: Boolean): String {
    ...
    return when (this) {
        ...
        is Expr.Bin -> {
            ...
            when (this.tk.str) {
                "++" -> {
                    ...
                    """
                    ({
                        ${tp.coder(TODO(),pre)} mar_$n = { .max=${tp.max!!.tk.str}, .cur=0 };
        ...
        is Expr.Call -> {
            ...
            val call = "${this.f.coder(pre)} ( ${this.args.mapIndexed { i,arg ->
                ...
                when {
                    ...
                    else -> "CAST(${tdst.coder(TODO(),pre)}, $src)"
                }
            }.joinToString(",")} )"
            """
            ({
        ...
        is Expr.Tuple  -> "((${this.typex().coder(TODO(),pre)}) { ${this.vs.map { (_,tp) -> "{"+tp.coder(pre)+"}" }.joinToString(",") } })"
        is Expr.Vector -> (this.typex() as Type.Vector).let {
            "((${it.coder(TODO(),pre)}) { .max=${it.max!!.tk.str}, .cur=${it.max!!.tk.str}, .buf={${this.vs.map { it.coder(pre) }.joinToString(",") }} })"
        }
        is Expr.Union  -> {
            val (i,_) = this.xtp!!.disc(this.idx)!!
            "((${this.typex().coder(TODO(),pre)}) { .tag=${i+1}, ._${i+1}=${this.v.coder(pre) } })"
        }
        is Expr.Field  -> {
        ...
        is Expr.Nat -> when {
            ...
            (this.xtp is Type.Data)    -> "CAST(${this.xtp!!.coder(TODO(),pre)}, ${this.tk.str})"
            else -> "((${this.xtp!!.coder(TODO(),pre)}) ${this.tk.str})"
        }
        ...
        is Expr.Throw -> """
            ({
                assert(sizeof(Exception) >= sizeof(${this.e.typex().coder(TODO(),pre)}));
                MAR_EXCEPTION = CAST(Exception, ${this.e.coder(pre)});
                continue;
                ${this.xtp!!.coder(TODO(),pre)} mar_$n ; mar_$n;
            })
        """

        is Expr.If -> "((${this.cnd.coder(pre)}) ? (${this.t.coder(pre)}) : (${this.f.coder(pre)}))"
        is Expr.MatchT -> """
            ${this.typex().coder(TODO(),pre)} mar_$n;
            switch (${this.tst.coder(pre)}.tag) {
                ${this.cases.map { (tst,e) -> """
                    ${tst.cond2({"case ${it.ts.coder(pre).uppercase()}_TAG"},{"default"})}:
                        mar_$n = ${e.coder(pre)};
