...
fun <V> Stmt.dn_collect_pos (fs: (Stmt)->List<V>, fe: (Expr)->List<V>, ft: (Type)->List<V>): List<V> {
    return when (this) {
        ...
        is Stmt.Await  -> (this.tp?.dn_collect_pos(fe,ft) ?: emptyList()) + (this.xpay?.dn_collect_pos(fe,ft) ?: emptyList())
...
fun <V> Stmt.dn_collect_pre (fs: (Stmt)->List<V>?, fe: (Expr)->List<V>?, ft: (Type)->List<V>?): List<V> {
    ...
    return v + when (this) {
        ...
        is Stmt.Await  -> (this.tp?.dn_collect_pre(fe,ft) ?: emptyList()) + (this.xpay?.dn_collect_pre(fe,ft) ?: emptyList())
