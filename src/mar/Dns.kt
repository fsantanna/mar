...
fun <V> Type.dn_collect_pos (ft: (Type)->List<V>): List<V> {
    return when (this) {
        ...
        is Type.Union -> (this.o?.dn_collect_pos(ft) ?: emptyList()) + this.ts.map { it.dn_collect_pos(ft) }.flatten()
...
fun <V> Type.dn_collect_pre (ft: (Type)->List<V>?): List<V> {
    ...
    return v + when (this) {
        ...
        is Type.Union -> (this.o?.dn_collect_pre(ft) ?: emptyList()) + this.ts.map { it.dn_collect_pre(ft) }.flatten()
