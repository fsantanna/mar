...
fun <V> Expr.dn_collect_pos (fe: ((Expr)->List<V>)?, ft: ((Type)->List<V>)?): List<V> {
    ...
    return when (this) {
        ...
        is Expr.Tuple -> this.vs.map { it.dn_collect_pos(fe,ft) }.flatten() + (this.xtp?.dn_collect_pos(ft) ?: emptyList())
        is Expr.Union -> this.v.dn_collect_pos(fe,ft) + (this.xtp?.dn_collect_pos(ft) ?: emptyList())
...
fun Expr.dn_visit_pos (fe: ((Expr)->Unit)?, ft: ((Type)->Unit)?) {
    this.dn_collect_pos (
        ...
        if (ft == null) null else ({ ft(it) ; emptyList<Unit>() }),
    )
}
